...
核心实现入口是 `q3/v6_policy.py:policy_v6`，frozen snapshot 是 `frozen/v6_n8_20260912/`。本地复现使用 `scripts/run_v6_eval.py`。

官方演练/正式测试建议使用干净入口 `run_q3_final.py`。它固定 RARC/n=8，不暴露历史版本选择，不硬编码参赛队号，并在结束时直接输出表 1 需要的清除干扰源个数、平均定位清除时间和程序运行时间。
